...
// TJpgDec Bytes per pixel of image output
#define JD_FORMAT 0/* RGB888 */
...
void loop()
{
  ...
  sprintf(fileName, "test%04d.jpg", seqNum); // change fileName to your file (8.3 format!)
...
byte doFTP(char buf[], uint32_t buf_length)
{
  ...
  if(!eRcv()) return 0;
  client.println(F("USER Your_FTP_Account"));

  if(!eRcv()) return 0;
  client.println(F("PASS Your_FTP_Password"));
  ...
  if(!eRcv()) return 0;
  client.println(F("CWD Your_Directory"));
